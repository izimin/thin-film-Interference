//
// Sample to show occlusion culling via ARB_occlusion_query extension and frame coherence
//
...
void	renderMajorX ( const Frustum& frustum, int incr )
{
	...
	for ( ; i >= 0 && i < N; i += incr )
	{
		...
		bool	anyVisible = false;
														...
														// render prev. visible cells from this row
		for ( j = jMin; j <= jMax; j++ )
			if ( cells [i][j] -> getVisToFrame () >= totalFrames )
			{
				cells [i][j] -> render ( totalFrames );
		...
		for ( j = jMin; j <= jMax; j++ )				// skip quering prev. visible cells
			if ( cells [i][j] -> getVisToFrame () < totalFrames )
			{
