...
void display ()
{
	...
	float	ambientColor [4] = { 1, 1, 1, 1 };		// white
	...
	glActiveTextureARB ( GL_TEXTURE2_ARB );		// bind decal map to texture unit 2
	glEnable           ( GL_TEXTURE_2D );
...
void key ( unsigned char key, int x, int y )
{
	...
	if ( key == 'p' || key == 'P' )
		saveScreenShot ( "rc-diffuse-3.tga" );
}
...
int main ( int argc, char * argv [] )
{
	...
	glutInitWindowSize  ( 500, 500 );
								...
								// create window
	int	win = glutCreateWindow ( "OpenGL per-pixel diffuse lighting with register combiners (atten) and decal" );
