	...
	for ( i = 0; i <= rings; i++ )
	{
      ...
      // T = normalize([dx/dtheta,dy/dtheta,dz/dtheta])
			...
			float	d        = 1 / sqrt ( dxdtheta*dxdtheta + dydtheta*dydtheta );
      		...
      		vertices [index].b.x = dxdtheta * d;
			vertices [index].b.y = dydtheta * d;
			vertices [index].b.z = 0;
		...
		// B = N cross T
			...
			vertices [index].t = -(vertices [index].n ^ vertices [index].b);
...
void	Torus :: draw ()
{
	...
	for ( int i = 0; i < numFaces; i++ )
		for ( int j = 0; j < 3; j++ )
		{
			...
			glMultiTexCoord3fv ( GL_TEXTURE1_ARB, v.l   );
			glMultiTexCoord2fv ( GL_TEXTURE2_ARB, v.tex );
		  	glVertex3fv        ( v.pos                  );
		}
